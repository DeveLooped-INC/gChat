...
import { AppRoute, UserProfile, Post, Message, Contact, ToastMessage, NetworkPacket, EncryptedPayload, Group, MediaMetadata, NodePeer, ConnectionRequest, NotificationItem } from './types';
...
const AuthenticatedApp = ({ user, onLogout, onUpdateUser }: { user: UserProfile, onLogout: () => void, onUpdateUser: (u: UserProfile) => void }) => {
  ...
  const addNotification = useCallback((title: string, message: string, type: ToastMessage['type'] = 'info', linkRoute?: AppRoute, linkId?: string) => {
    ...
    const newNotification: NotificationItem = { id, title, message, type, timestamp: Date.now(), read: false, linkRoute, linkId };
  ...
  }, [handleNotificationNavigation, state.setNotifications]);
  ...
  }, [state.groupsRef, state.setGroups, state.userRef, state.contactsRef]);

  const handleLeaveGroup = useCallback((groupId: string) => {
      const user = state.userRef.current;
      const group = state.groupsRef.current.find(g => g.id === groupId);
      if (!group) return;
      if (group.ownerId === user.id) { addNotification('Action Denied', 'Owners cannot leave. Delete the group instead.', 'error'); return; }
      state.setGroups(prev => prev.filter(g => g.id !== groupId));
      if (group) {
  ...
  }, [state.setGroups, state.userRef, state.contactsRef]);
  ...
  }, [state.groupsRef, state.userRef, state.setGroups, state.contactsRef]);
  ...
  }, [broadcastPostState, state.setPosts, state.contactsRef, state.userRef]);
  ...
  }, [broadcastPostState, state.setPosts, state.userRef, state.peersRef, processedPacketIds]);
  ...
  const handleDeletePost = useCallback((postId: string) => {
      ...
      processedPacketIds.current.add(packet.id!);
      networkService.broadcast(packet, state.peersRef.current.map(p => p.onionAddress));
  }, [state.setPosts, state.userRef, processedPacketIds, state.peersRef]);
  ...
  }, [broadcastPostState, state.userRef, state.setPosts, processedPacketIds, state.peersRef]);
  ...
  const handleCommentReaction = useCallback((postId: string, commentId: string, emoji: string) => {
      ...
      const packet: NetworkPacket = { id: crypto.randomUUID(), hops: MAX_GOSSIP_HOPS, type: 'COMMENT_REACTION', senderId: user.homeNodeOnion, payload: { postId, commentId, userId: user.id, emoji } };
      processedPacketIds.current.add(packet.id!);
      networkService.broadcast(packet, state.peersRef.current.map(p => p.onionAddress));
      if (updatedPostForBroadcast) broadcastPostState(updatedPostForBroadcast);
  }, [broadcastPostState, state.userRef, state.setPosts, processedPacketIds, state.peersRef]);
      ...
      >
          ...
          {activeRoute === AppRoute.NODE_SETTINGS && (
              <NodeSettings 
                  ...
                  data={{ posts: state.posts, messages: state.messages, contacts: state.contacts }}
              />
          )}

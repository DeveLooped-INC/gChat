...
import { Contact, Message, ToastMessage, UserProfile, Group, MediaMetadata, Post } from '../types';
...
const Chat: React.FC<ChatProps> = ({ 
  ...
  const submitCreateGroup = () => {
     if(!newGroupName.trim()) { addToast("Error", "Group Name required", 'error'); return; }
     if(selectedMembers.size === 0) { addToast("Error", "Select at least 1 member", 'error'); return; }
             ...
             {threadMessages.map((msg) => {
               ...
               <div id={`msg-${msg.id}`} key={msg.id} className={`flex ${msg.isMine ? 'justify-end' : 'justify-start'} animate-in slide-in-from-bottom-2 duration-300 group`}>
                 <div className="flex items-end gap-2 max-w-[85%] md:max-w-md">
                    {!msg.isMine && (
                        <div onClick={() => openUserInfo(msg.senderId)} className="w-8 h-8 rounded-full bg-slate-800 flex items-center justify-center text-xs font-bold text-slate-300 shrink-0 cursor-pointer hover:bg-slate-700 transition-colors mb-1">

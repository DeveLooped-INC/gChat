...
const Onboarding: React.FC<OnboardingProps> = ({ onComplete }) => {
  ...
  const handleResetOwner = () => {
      if(confirm("This will clear the 'Node Owner' lock. The next user to login will become the Admin. Do this only if you are locked out of settings.")) {
          localStorage.removeItem('gchat_node_owner');
  ...
  const finalizeUser = async (keys: any, chosenHandle: string, finalAvatar?: string) => {
      try {
          ...
          let nodeOwnerId = localStorage.getItem('gchat_node_owner');
          let isAdmin = false;

          if (!nodeOwnerId) {
              localStorage.setItem('gchat_node_owner', userId);
              nodeOwnerId = userId;
              isAdmin = true;
          ...
          localStorage.setItem('gchat_user_profile', JSON.stringify(profile));
              ...
              >
                {logs.map((log, i) => (
                  <div key={i} className={`mb-1 break-all ${
                      log.includes('ERROR') ? 'text-red-500' : 'text-slate-400'
                  }`}>
                    {log}
                  </div>
                ))}
